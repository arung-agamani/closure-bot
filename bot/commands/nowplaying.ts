...
export function execute(
  ...
  NowPlayingEmbed.setDescription(
    `Showing queue of ${musicQueue.songs.length} items`
  );
  for (let i = 0; i < musicQueue.songs.length; i++) {
    if (i === musicQueue.currentlyPlaying) {
      NowPlayingEmbed.addField(
        `**${i + 1}. ${musicQueue.songs[i].title}**`,
        musicQueue.songs[i].url
      );
    } else {
      NowPlayingEmbed.addField(
        `${i + 1}. ${musicQueue.songs[i].title}`,
  ...
  NowPlayingEmbed.addField(
    `Currently playing on ${musicQueue.currentlyPlaying + 1}`,
    musicQueue.songs[musicQueue.currentlyPlaying].title
  );
