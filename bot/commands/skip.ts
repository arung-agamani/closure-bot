...
import { MusicQueue } from '../commands/nowplaying';
...
export function execute(message: Message, args: Array<string>, botObject: ClosureType.default) {
    ...
    if (args.length == 2) {
        if (args[0] === 'to' && validRegex.test(args[1])) {
            const jumpPos = parseInt(args[1]);
            const musicQueue: MusicQueue = botObject.musicQueue.get(message.guild.id);
            if (jumpPos > musicQueue.songs.length || jumpPos < 1) {
                return message.channel.send("Index out of queue range. Enter valid position >:(");
            }
            musicQueue.currentlyPlaying = jumpPos - 2;
            musicQueue.connection.dispatcher.end();
            return message.channel.send(`Skipping to pos ${jumpPos}: **${musicQueue.songs[jumpPos-1].title}**`);
        } else {
            return message.channel.send("Wrong command. Use ```skip to <jump position>``` as the flag");
        }
