    ...
    if (tagsXHR.readyState == XMLHttpRequest.DONE) {
        ...
        for (const tagObj of tagsContainer.tags) {
            ...
            chrome.contextMenus.create({
                ...
                parentId : "WARFARIN_001",
                contexts : ["image", "link"],
                documentUrlPatterns : ["https://*.pixiv.net/ */ function checkOrigin(
  pageUrl
) {
  if (pageUrl.match('facebook')) {
    return 'Facebook';
  }
  if (pageUrl.match('twitter') || pageUrl.match('twimg')) {
    return 'Twitter';
  }
}
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        ...
        alert(
          `There is a problem with the XHR : ${xhr.status} : ${xhr.statusText}`
        );
      }
    }
  };
...
function loadTags() {
  chrome.storage.local.get('currentUsedServer', (serverInfo) => {
    if (
      serverInfo.currentUsedServer.guildId !== undefined &&
      serverInfo.currentUsedServer.guildId.match(/^\d*$/)
    ) {
      ...
      console.log(pointedGuildId);
      const { tags } = serverInfo.currentUsedServer;
