  ...
  start(): void {
    this.socketIOServer.on('connection', (socket) => {
      ...
      socket.on('init url', (data) => {
        ...
        if (jobCount >= 3) {
          socket.send(
            'Maximum 3 concurrent jobs. Please finish your current jobs before adding new job.'
          );
        } else if (ytRegex.test(incomingUrl)) {
          ...
          ytdl
            ...
            .then((videoInfo) => {
              ...
              if (videoInfo.formats.some((frmt) => frmt.isLive)) {
                ...
                socket.send(
                  'Video is currently live. Cannot process that. Aborting.'
                );
                ...
                const videoName = videoInfo.videoDetails.title.replace(
                  // eslint-disable-next-line no-useless-escape
                  /[\/|\|]/g,
                  ' - '
                );
                ...
                const videoReadableStream = ytdl(incomingUrl, {
                  filter: 'audioonly',
                ...
                stream.on('finish', () => {
                  ...
                  const timemarkRegex = new RegExp(
                    /^\d\d:(\d\d):(\d\d)\.\d\d$/
                  );
                  ...
                  try {
                    ...
                    ffmpeg = FFMPEG(fs.createReadStream(filePath)).toFormat(
                      'mp3'
                    );
                    ffmpeg
                      .once('progress', () => {
                        conversionHandler = setInterval(() => {
                          socket.emit('conversion', {
                            progress: processedSecond / videoLength,
                            id: videoInfo.videoDetails.videoId,
                          });
                        }, 1000);
                      })
                      .on('progress', (progress) => {
                        ...
                        processedSecond =
                          parseInt(timemarks[1]) * 60 + parseInt(timemarks[2]);
                      })
                      .on('end', () => {
                        ...
                        this.map.set(
                          hashfn,
                          path.resolve(destUrl, `${videoName}.mp3`)
                        );
                        socket.emit('done', {
                          filename: videoName,
                        ...
                        setTimeout(() => {
                          ...
                          console.log(
                            `${videoName}.mp3 is now expired and deleted`
                          );
                        }, 300 * 1000);
                        console.log(
                          'Finished converting. Now serving until expired'
                        );
                        jobCount -= 1;
                      })
                      .on('error', (ffmpegErr) => {
                    ...
                    socket.emit('error', {
                      message: 'Error on mp3 conversion task.',
                      type: 'convert',
                      id: videoInfo.videoDetails.videoId,
                    });
                ...
                stream.on('error', (streamErr) => {
                  ...
                  socket.emit('error', {
                    message: 'Error when data streaming',
                    type: 'stream',
                    id: videoInfo.videoDetails.videoId,
                  });
                  stream.close();
            ...
            .catch((err) => {
              socket.emit('error', {
                message: 'Error on fetching info',
                type: 'fetch',
                id: incomingUrl,
              });
